...
export class RouterDetails extends Component {
  ...
  handleSubmit = async event => {
    ...
    await fetch(Config.serverAddress + '/api/v1/routers/' + this.state.id, {
      method: 'PUT',
      headers: {
  ...
  componentDidMount() {
    ...
    let url = Config.serverAddress + "/api/v1/routers/" + this.props.match.params.id;
  ...
  render() {
    ...
    if (error) {
      ...
      return (
        <div>
          <span class="d-block p-2 bg-primary text-white">Szczegółowe dane routera</span>
          <form class={document.getElementsByClassName("is-invalid").length == 0 ? "was-validated" : ""}>
            <div class="form-group">
              <label for="deviceID" >ID urządzenia</label>
              <input type="number" placeholder="Wpisz ID routera" class="form-control is-valid " id="deviceID" defaultValue={this.state.deviceId} disabled={this.state.formDisabled} onChange={(event) => {
              ...
              <select class="form-control is-valid" id="exampleFormControlSelect1" required defaultValue={this.state.owner} disabled={this.state.formDisabled} onChange={(event) => {
            ...
            <input type="text" placeholder="Wpisz producenta routera" class="form-control is-valid" id="producer" defaultValue={this.state.producer}  required disabled={this.state.formDisabled} onChange={(event) => {
            ...
            <input type="text" placeholder="Wpisz model routera" class="form-control is-valid" id="model" defaultValue={this.state.model} required disabled={this.state.formDisabled} onChange={(event) => {
          ...
          <Modal header="Sukces" body={"Zaktualizowano router ID: "+ this.state.deviceId} id="modalSuccess" onCloseClicked={()=>window.location.href='/routers'} />
